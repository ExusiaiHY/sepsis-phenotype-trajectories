...
- A **supplementary downstream mortality validation** showing that frozen S1.5 embeddings support held-out discrimination (AUROC 0.829 on Center B), while clarifying why raw accuracy is misleading at 14% event prevalence
...
**Table 2b. Supplementary downstream mortality classification on frozen S1.5 embeddings**

| Operating Point | Test Accuracy | Test Balanced Accuracy | Test Precision | Test Recall | Test F1 | Test AUROC |
|----------------|--------------:|-----------------------:|---------------:|------------:|--------:|-----------:|
| Validation-balanced threshold (`thr=0.55`) | 0.784 | 0.745 | 0.372 | 0.691 | 0.484 | 0.829 |
| Validation-accuracy threshold (`thr=0.85`) | 0.865 | 0.623 | 0.582 | 0.280 | 0.378 | 0.829 |
| Majority-class baseline | 0.854 | 0.500 | - | 0.000 | - | - |

A logistic-regression classifier trained on frozen S1.5 embeddings achieved held-out Center B AUROC 0.829. When the operating threshold was selected to maximize validation balanced accuracy, test balanced accuracy reached 0.745 with recall 69.1%. When the threshold was instead chosen to maximize plain validation accuracy, test accuracy rose to 0.865 but recall collapsed to 28.0%, only marginally improving over the majority-class baseline accuracy of 0.854. This comparison shows that the learned embedding space contains usable prognostic information, but also that plain accuracy is a poor standalone summary under severe class imbalance.
...
The supplementary frozen-embedding mortality classifier is consistent with that interpretation. On the held-out Center B split, the S1.5 space supported AUROC 0.829 without any end-to-end supervised fine-tuning. At the same time, the threshold-ablation result shows why this paper does not frame the project as a conventional mortality-prediction benchmark: once event prevalence falls to roughly 14%, raw accuracy can look strong even when recall is clinically weak. The more meaningful signal is that the same representation supports both trajectory analysis and out-of-center mortality discrimination when evaluated with imbalance-aware metrics.
...
4. **Earlier bedside deployment**: The newly added frozen-embedding mortality classifier shows that the learned space is operationally usable. A practical next step would infer phenotype membership or mortality risk from 6-hour or 12-hour windows and update predictions online as new measurements arrive.
...
The main scientific lesson is that explicit handling of structured missingness and temporal locality is essential for ICU time-series phenotyping. Static feature clustering can reveal coarse heterogeneity, but mask-aware self-supervised representation learning is what makes rolling-window analysis feasible. The supplementary downstream validation added here further shows that the same frozen embedding space supports held-out mortality discrimination (AUROC 0.829 on Center B), though class imbalance makes plain accuracy an unreliable standalone metric. The present results are descriptive rather than causal, yet they show that temporal phenotype trajectories carry reproducible clinical information beyond static subtyping. That makes them a plausible foundation for future treatment-aware, externally validated, and eventually bedside-deployable sepsis phenotyping systems.
