...
A configurable synthetic data generator with known ground-truth subtype labels enables quantitative evaluation of clustering recovery (ARI, NMI). In addition, we built a local bridge from the PhysioNet/CinC 2019 Sepsis Challenge stubs [9], yielding 40,331 ICU stays in the same on-disk format as `data/s0`. Eighteen of the 21 continuous PhysioNet 2012 channels map directly (`gcs`, `sodium`, and `pao2` are unavailable in the 2019 source), and PhysioNet 2012 preprocessing statistics are reused so the auxiliary source is numerically compatible with the pretrained encoder. A MIMIC-IV mock database (15 patients, 31 features) continues to serve as a schema-compatibility smoke test only; an automated DuckDB profile additionally verified that all 9 tables required by the legacy analysis-table builder are present in the local mock database. During this revision we also added demo-ready ingestion scripts for local MIMIC-IV-demo and eICU-CRD-demo CSVs, plus a raw-table eICU loader that maps vitals, labs, and organ-support proxies into the same tensor interface. However, on March 24, 2026, the official PhysioNet demo pages for MIMIC-IV-demo 2.2 and eICU-CRD-demo 2.0.1 returned region-restricted `Data Not Available` responses from our execution environment, so no real demo-derived external-validation metrics are claimed in this paper.
...
5. **Single-dataset scope**: Cross-center temporal validation within the PhysioNet 2012 cohort is strong internal evidence, but both centers derive from the same source database. The repository now includes demo-ready MIMIC/eICU ingestion code, yet real external validation on independently collected ICU cohorts is still pending because official demo files were not accessible from the current environment on March 24, 2026.
